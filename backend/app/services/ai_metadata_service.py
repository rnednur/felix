...
class AIMetadataService:
    ...
    async def generate_column_descriptions(
        ...
        """
        Use AI to generate descriptions for all columns based on their names,
        data types, ranges, and categorical values.

        Only provides descriptions when they add value beyond what's self-explanatory.
        """
        # Load schema and stats
        schema = self.storage_service.load_schema(dataset_id)

        # Build smart prompt
        prompt = self._build_description_prompt(schema)

        # Call AI
        response = await self._call_ai(prompt)

        # Parse response
        descriptions = self._parse_descriptions_response(response)
    ...
    def _build_description_prompt(self, schema: dict) -> str:
        """Build prompt for AI column description generation"""
        columns_info = []
        for col in schema['columns']:
            ...
            if 'stats' in col:
                if 'top_values' in col['stats']:
                    ...
                    col_info['categorical_values'] = [str(v[0]) for v in col['stats']['top_values'][:10]]
                ...
                if 'mean' in col['stats']:
                    col_info['mean'] = col['stats']['mean']
                if 'null_count' in col['stats']:
                    col_info['null_count'] = col['stats']['null_count']

            columns_info.append(col_info)

        return f"""You are a data analyst helping to automatically describe dataset columns.

Dataset columns with statistics:
{json.dumps(columns_info, indent=2)}

IMPORTANT INSTRUCTIONS:
1. ONLY provide descriptions when they add meaningful context beyond what's obvious from the column name
2. For self-explanatory columns (like "email", "phone", "name", "id"), return NULL or skip them
3. Focus on columns where the description adds business context, clarifies ambiguity, or explains data patterns
4. Use the data type, ranges, and categorical values to infer meaning
5. Be concise - descriptions should be 1-2 sentences maximum

Examples of GOOD descriptions (add value):
- "customer_lifetime_value" → "Total revenue generated by a customer over their entire relationship with the company"
- "nps_score" → "Net Promoter Score ranging from -100 to 100, measuring customer loyalty and satisfaction"
- "cohort_month" → "The month when the user first signed up or made their first purchase"

Examples of BAD descriptions (don't add these):
- "email" → "User's email address" (obvious, skip it)
- "first_name" → "The first name of the user" (obvious, skip it)
- "created_at" → "Timestamp when record was created" (obvious, skip it)

Output ONLY a JSON array of column suggestion objects with these fields:
- column: string (column name)
- description: string (concise description that adds value)
- business_definition: string (optional - only if there's important business context)
- semantic_type: string (optional - one of: email, phone, url, currency, percentage, date, datetime, identifier, category, metric)
- examples: array (optional - up to 3 representative values if helpful)

Only include columns where you can add meaningful value. If a column is self-explanatory, don't include it.
...
JSON output:"""
    ...
    def _parse_descriptions_response(self, response: str) -> List[Dict[str, Any]]:
